...
const Faq = ({questions}) => {
  ...
  return (
    <div className="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 space-y-1 lg:px-8 lg:py-20">
      <h1 data-aos="zoom-in" className="text-4xl font-bold text-center text-primary">
        Frequently Asked Questions
      </h1>
      <p data-aos="fade-up" className="mb-14 text-center w-3/4 mx-auto text-secondary mt-6">
        ...
        {questions?.map((q) => (
          <div key={q.id}>
            <div className="collapse collapse-arrow bg-neutral border border-base-300">
              ...
              <div className="collapse-content text-secondary">{q.answer}</div>
            </div>
          </div>
        ))}
      </div>
